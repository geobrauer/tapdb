...
`select deva(sanskrit), def`  
where `sanskrit` and `def` are columns, and `deva()` specifies that the Sanskrit column should be rendered in a given font appropriate to the Devanāgarī script.
